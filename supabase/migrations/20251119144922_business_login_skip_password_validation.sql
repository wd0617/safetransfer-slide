/*
  # Business Login - Skip Password Validation
  
  1. Changes
    - Remove password validation from business_login function
    - Password validation now happens in frontend using bcrypt
    - This function only handles session creation and business status checks
    
  2. Security
    - Frontend validates bcrypt password hash
    - This function still checks business status and creates secure sessions
    - Maintains SECURITY DEFINER for session creation
*/
...
DECLARE
  ...
  v_session_id UUID;
BEGIN
  -- Find admin by email (password already validated in frontend)
